...
import (
	...
	"github.com/10nates/wordpos" // Yes, I made my own version of wordpos just for this because I couldn't find an alternative.
)
...
func generateSentenceFromSymbol(symbol string) string {
	...
	for _, part := range strings.Split(rule, " ") {
		switch part {
		case "N":
			words, err := wordpos.RandNoun("", 1)
			if err != nil || len(words) == 0 {
				fmt.Println(err)
				sentence += "ERROR " // What are you gonna do about it?
			} else {
				sentence += words[0].Word + " "
			}
		case "V":
			words, err := wordpos.RandVerb("", 1)
			if err != nil || len(words) == 0 {
				fmt.Println(err)
				sentence += "ERROR " // What are you gonna do about it?
			} else {
		...
		case "Adj":
			words, err := wordpos.RandAdjective("", 1)
			if err != nil || len(words) == 0 {
				fmt.Println(err)
				sentence += "ERROR " // What are you gonna do about it?
			} else {
		...
		case "Adv":
			words, err := wordpos.RandAdverb("", 1)
			if err != nil || len(words) == 0 {
				fmt.Println(err)
				sentence += "ERROR " // What are you gonna do about it?
			} else {
...
func generateTidiedSentence() string {
	sentence := generateSentenceFromSymbol("S")
	// remove underscores
